...
export class WindowManager {
    constructor({ rootEl, api, ctx, onChange, eventBus, host, taxonomy, events, content,
         ...
         *  that list. See `_returnToOpenList`. Default off: it changes what the key
    ...
    navigateBack() {
        ...
        // canonical parent instead, in place. An ENTITY-level parent wins (the
        // instance one level up); failing that, the KIND-level parent.
    ...
    navigateUp(kind, props = {}, { ctx = null } = {}) {
        ...
        const active = this._scopeActiveTab(scope);
        const aimsAtOwnList = !_isEntityTab({ props })
            && _isEntityTab(active)
            && this.taxonomy.topNavFor(kind) === this.taxonomy.topNavFor(active.kind);
    ...
    _scopeTabs(scope) {
        if (scope?.windowId) {
            ...
            return { tabs, activeIdx: Math.max(0, Math.min(tabs.length - 1, rec.activeTabIdx || 0)) };
        }
    ...
    _popTabHistory(id) {
        ...
        if (prior.topNav && curTopNav && prior.topNav !== curTopNav) {
            tree.swapToPage?.(id, { kind: prior.kind, props: prior.props || {}, title },
                curTopNav, prior.topNav, { recordHistory: false });
        } else {
            tree.replaceActiveTabContent(id, { kind: prior.kind, props: prior.props || {} },
                title, { recordHistory: false });
        }
        tree.focus(id);
        this.renderer.render();
        this._persist();
    ...
    _returnToOpenList(scope) {
        ...
        if (own >= 0 && tabs.length > 1) {
            ...
            const tree = this._tree();
            tree.removeLeafTab(scope.leafId, activeIdx);
            tree.setActiveLeafTab(scope.leafId, landing);
            tree.focus(scope.leafId);
            this.renderer.render();
            this._persist();
        ...
        for (const leaf of ordered) {
            ...
            if (Array.isArray(rec.tabs) && rec.tabs.length > 1) {
                ...
                try { rec.window.close({ force: true }); } catch { /* already gone */ }
            }
    ...
    openInPrimary(kind, props = {}) {
        ...
        // C35. THE SAME PAGE WITH DIFFERENT PROPS IS A DIFFERENT VIEW. swapToPage
        // finds an existing tab by kind + props.id, and a list has no id, so a
        // filter clicked while that list is showing matched the open list tab
        // and re-activated it unchanged: the click did nothing. The active tab
        // must carry the requested props. A BARE open (no props, a top-nav
        // click) is left alone, because putting back the tab you left, filter
        // and all, is the point of archiving page tabs. BugDesk's own tiling
        // fork carried this correction; it is upstream now.
        const _leaf = tree.get(leafId);
        const _active = _leaf?.tabs?.[_leaf.activeTabIdx];
        if (_active && _active.kind === kind && Object.keys(props || {}).length > 0
...
/** The index nearest to `from` whose tab passes `test`: leftward first, then
 *  rightward. -1 when none does. */
